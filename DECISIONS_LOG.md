...
- **Not applied to production yet:** `supabase/migrations/20260925_audit_invoice_ids_and_comment_limits.sql` (adds `tips.invoice_id` for exactly-once tips, and length limits on `comments`). The code works with or without it. A direct DB migration was refused by the session's permission classifier, so Ian must run it in the Supabase SQL editor.
