...
| 2026-09-25 | Set Vic's IntaSend live keys in Vercel + merged `feat/intasend-vic-account` → `main` (`0dbd342`). Set new random `ADMIN_PASSWORD` + `ADMIN_TOKEN_SECRET` in Vercel + merged `security/remove-admin-password-fallback` → `main` (`75c5368`). Both deploys pushed to GitHub. Pending human steps: Ian sends Vic new admin password; payment smoke tests (soda tip + paid-entry unlock + rejection test). |
