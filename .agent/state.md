...
| 2026-08-22 | Session 16: Deep Security Audit & Hardening — Conducted comprehensive security review across frontend, API routes, authentication, input sanitization, and dependencies. (1) Patched Stored XSS in comment rendering and markdown formatting via `escapeHTML()` entity encoding. (2) Hardened `/api/stk-push` serverless endpoint with amount range validation (KES 10 - 500,000) and strict Kenyan phone regex sanitization. (3) Added standard HTTP security headers to `vercel.json` (`X-Frame-Options: DENY`, `X-Content-Type-Options: nosniff`, `Referrer-Policy: strict-origin-when-cross-origin`, `Permissions-Policy`). Clean build verified and deployed to production on GitHub `main`. |
| 2026-08-22 | Session 17: SEO, Crawlability & Production Alignment — (1) Corrected Schema.org JSON-LD microdata in `index.html` with real live Vercel domain (`https://the-villagers-notes.vercel.app`), real contact email (`vikmunala@gmail.com`), and verified social links. (2) Updated `public/llms.txt` with accurate author bio, project overview, WhatsApp, and email. (3) Created and deployed `public/robots.txt` and `public/sitemap.xml` for Google Search Console and AI engine crawling. Clean build verified and deployed to production on GitHub `main`. |
| 2026-08-22 | Session 18: Admin Top Bar Sticky Fix & Customizable Free Preview Paragraphs — (1) Fixed Admin dashboard top bar sticky positioning: hid public site-nav on `#/admin` route and locked Admin tab bar to `position: sticky; top: 0; z-index: 999;` with solid opaque background and subtle shadow, preventing scrolled content from bleeding through or creating gaps. Added a `← View Site` pill button to quickly navigate back to the public website. (2) Added a `Free Preview (Paragraphs)` numeric field in New Entry / Edit Entry forms, allowing Vik to choose exactly how many paragraphs (1, 2, 3, etc.) are free before the M-Pesa paywall appears. Upgraded reader to dynamically honor `entry.previewCount`. Clean build verified and pushed to production on GitHub `main`. |
| 2026-08-24 | Session 19: Client Meeting Refinements & Feature Upgrades — (1) Restored clean handwritten "The Villager's Notes" text wordmark logo in header with responsive styling. (2) Restored share dropdown menu on story pages (Copy Link, Share on X, Facebook, WhatsApp) with outside-click dismissal. (3) Upgraded comments on story pages with open inline textarea, 500-character limitation, and live countdown indicator. (4) Completely removed the reading progress line across all templates and stylesheets. (5) Added glassmorphism / frosted background opacity (`backdrop-filter: blur(12px)`) to sticky header when scrolled. (6) Constrained all content dividers within the central max-width container, reserving full-width edge-to-edge rules exclusively for the header bottom and footer top. (7) Removed WhatsApp entirely (FAB, book order link, and error message references). (8) Removed "City" field from newsletter signup. (9) Implemented inline accordion book order on Projects page with full name, M-Pesa number, address, signed copy request, and STK push. (10) Removed "JULY 2026" duplicate dates and "delivered to your door" copy. (11) Added clean editorial stats bar for the play (duration, staged location, structure, digital stream). (12) Removed `·admin` link from footer. (13) Conducted comprehensive domain research across `vicmunala.com`, `thevillagersnotes.com`, and `.co.ke`. Clean build verified. |
| 2026-08-24 | Session 20: Visual Refinement & Supabase Engagement Stats Integration — (1) Matched "WATCH THE TRAILER →" button to minimalist outlined style (`btn--sharp`), removing dominant orange fill. (2) Constrained Projects page header divider within `.container`, eliminating edge-to-edge line. (3) Removed unnecessary horizontal divider rule under the Home hero quote. (4) Built minimalist glass pill Share button with dropdown menu (classic Twitter with bird logo, Facebook, WhatsApp, Copy link) and outside-click dismissal. (5) Installed `@supabase/supabase-js`, created `src/lib/supabase.js`, and integrated live engagement metrics into private Admin Stats dashboard while preserving editorial play details publicly. (6) Fixed literal `<br/>` tags in article story paragraphs by safely parsing `<br />` and `\n` line breaks in `formatInline()`. Clean build verified. |
...
- When an author removes WhatsApp from public display to protect privacy, provide seamless self-service M-Pesa checkout inline so conversion rates remain high without manual chat overhead.
