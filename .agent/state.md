...
| 2026-08-21 | Session 13: Round 2 Polish & Client Adjustments — (1) Fixed home hero quote line breaks to place "a" on the 3rd line with explicit `<br>` and wrapped section to make the bottom divider rule span full viewport edge-to-edge. (2) Removed search bar from Entries page. (3) Added top fixed CSS styles for reading progress bar on story view. (4) Removed italics from story excerpt/standfirst. (5) Cleaned up story comments (removed "No comments yet" empty state text, converted inputs to bottom-underlined editorial fields). (6) Swapped terracotta style to Watch Trailer button, kept both play CTAs on one line, and reduced soda section divider to a single line. (7) Removed "more in the drawers" line from Projects header. (8) Removed "2024" year and "DELIVERED" from novel metadata. (9) Removed "TWO-HANDER" and "SCRIPT & RIGHTS AVAILABLE" from play metadata (retained "77 MINUTES"). (10) Set "Write to me" section to 2 columns on desktop. (11) Swapped header text logo for `VN Logo.png` with multiply blend mode. (12) Redesigned `favicon.svg` with corner-bracket branding motif. Clean build verified and pushed to production on GitHub `main`. |
